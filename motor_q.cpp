...
//从站配置所用的参数
#define EP3ESLAVEPOS 6, 0               //迈信伺服EP3E在ethercat总线上的位置
...
init_EtherCAT_master(struct MOTOR *motor) {
    ...
    ec_pdo_entry_reg_t domain_regs[] = {
        ...
        {EP3ESLAVEPOS, MAXSINE, TARGET_TORQUE, 0,&motor->drive_variables.target_torque},
        ...
        {EP3ESLAVEPOS, MAXSINE, MAX_TORQUE, 0,&motor->drive_variables.max_torque},
        ...
        {EP3ESLAVEPOS, MAXSINE, CURRENT_TORQUE, 0,&motor->drive_variables.current_torque},
        {EP3ESLAVEPOS, MAXSINE, CURRENT_VELOCITY, 0,&motor->drive_variables.current_velocity},
        {EP3ESLAVEPOS, MAXSINE, CURRENT_POSITION, 0,&motor->drive_variables.current_postion},
        {}};
...
int main(int argc, char **argv) 
{
    ...
    motor.targetVelocity = 300000;
    motor.targettorque = 200;
    motor.opModeSet = 0xa;     //速度模式
	...
	while (true){
        ...
        motor.currenttorque = EC_READ_S32(
            motor.domain_pd + motor.drive_variables.current_torque);  //当前力矩
		...
		//printf("EP3E:  act position = %d , act velocity = %d , act torque = %d ,status = 0x%x , "
        //       "drivestate = %d , opmode = 0x%d\n",
        //       motor.currentPosition, motor.currentVelocity,motor.currenttorque , motor.status,
        ...
        if(motor.resetBusy == true) {
            ...
            switch(motor.driveState) {
                ...
                case dsSwitchOnDisabled:
                    ...
                    EC_WRITE_U16(motor.domain_pd + motor.drive_variables.ctrl_word,
                                 0x0006);
                    EC_WRITE_U16(motor.domain_pd + motor.drive_variables.max_torque,
                                 0x0BB8);
                    break;
                ...
                case dsSwitchedOn:
                   ...
                   //     motor.currentPosition;  //将当前位置复制给目标位置，防止使能后电机震动
        ...
        if(motor.driveState == dsOperationEnabled && motor.resetBusy == 0 &&
           motor.powerBusy == 0 && motor.quickStopBusy == 0) {
            if(motor.opmode == 10) {           //力矩模式
            EC_WRITE_S32(motor.domain_pd + motor.drive_variables.target_torque,
                                 motor.targettorque);
