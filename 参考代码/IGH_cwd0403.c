...
{ 
    ...
    // int out = open("test.txt", O_RDWR|O_CREAT|O_APPEND, 0600);
    // if (-1 == out) { perror("opening cout.log"); return 255; }
    // ftruncate(out,0);//清空文件
    // int save_out = dup(fileno(stdout));
    // if (-1 == dup2(out, fileno(stdout))) { perror("cannot redirect stdout"); return 255; }

    motor.targetPosition = 0;
    //motor.opModeSet = 8;         //位置模式
    motor.opModeSet = 9;         //速度模式
    motor.powerBusy=1;
    ...
    while (true){
        ...
        printf("电机运行模式%d||||电机当前速度%d||||当前位置%d\n",motor.opmode,motor.currentVelocity,motor.currentPosition);
            ...
            switch(motor.driveState) {
                ...
                case dsSwitchOnDisabled:
                    ...
                    EC_WRITE_U16(motor.domain_pd + motor.drive_variables.ctrl_word,
                                    0x0006);
                    printf("dsSwitchOnDisabled\n");
                    break;
                case dsReadyToSwitchOn:
                    EC_WRITE_U16(motor.domain_pd + motor.drive_variables.ctrl_word,
                                    0x0007);
                    printf("dsReadyToSwitchOn\n");               
                    break;
