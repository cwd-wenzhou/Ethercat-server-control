...
//从站配置所用的参数
#define EP3ESLAVEPOS 0, n               //迈信伺服EP3E在ethercat总线上的位置
...
static void init_EtherCAT_master(struct MOTOR_MASTER *motor_master){
    //创建ethercat主站master
    motor_master->master = ecrt_request_master(0);
    if(!motor_master->master) {
        printf("Failed to create ethercat master!\n");
        exit(EXIT_FAILURE);  //创建失败，退出线程
    }
}

static void activate_EtherCAT_master(struct MOTOR_MASTER *motor_master){
    //激活主站master
    printf("Activating master...\n");
    if(ecrt_master_activate(motor_master->master)) {
...
//初始化EtherCAT函数
static void
init_EtherCAT_slave(struct MOTOR_MASTER *motor_master,struct MOTOR *motor_master,int n) {
    ...
    //创建域domain
    motor_master->domain = ecrt_master_create_domain(motor_master->master);
    if(!motor_master->domain) {
...
void EtherCAT_mainloop(struct MOTOR motor,struct MOTOR_DATA motor_data){
        static int count;
        static int count=0;
        static int8_t reset_count = 0;      //复位的时候计数用
        static int16_t position_count = 0;  //位置模式下计数
        ...
        if (count>1000){
            count=0;
            for (int i=0;i<num;i++){
                printf("第%n台：电机实时位置%d\t,电机目标位置%d\n",i,motor_data[i].currentPosition,motor_data[i].targetPosition);
            }
...
{
    struct MOTOR motor[num];
    struct MOTOR_DATA motor_data[num];
    struct MOTOR_MASTER motor_master;
    printf("*It's working now*\n");
    init_EtherCAT_master(&motor_master);
    for (int i=0;i<num;i++){
        motor_data[i].targetPosition = 0;
        motor_data[i].powerBusy=1;
        motor_data[i].positionMoving=1;
        motor[i].opModeSet = 8;
        //位置模式
        init_EtherCAT_slave(&motor_master,&motor[i],i);
    }
    activate_EtherCAT_master(&motor_master);
       
    while (true){
        usleep(1000);
        //接收过程数据
        ecrt_master_receive(moster_master.master);
        
        //检查过程数据状态（可选）
        //check_domain_state(moster_master.domain, &moster_master.domain_state);
        //检查主站状态
        check_master_state(moster_master.master, &moster_master.master_state);
        for (int i=0;i<num;i++){
            EtherCAT_mainloop(motor[i],motor_data[i]);
        }
        //发送过程数据
        ecrt_master_send(moster_master.master);
    }
