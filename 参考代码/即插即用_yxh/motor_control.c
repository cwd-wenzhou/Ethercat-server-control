            ...
            {   //位置模式
                ...
                { //开始回零
                    ...
                    else if (motor.currentPosition == 0)
                    {
                        ...
                        motor.positionMoving = true; //回零结束，开始运动
                        position_count = 0;
                        t = 0;
                    }
                    EC_WRITE_S32(motor.domain_pd + motor.drive_variables.target_postion,
...
void post_message(){
    system("curl http://192.168.0.21 -X POST -d 'parameterName1=parameterValue1&parameterName2=parameterValue2' ");
}
int main(int argc, char **argv)
{
    ...
    FILE *fp;
    char buffer[800];
    fp = popen("ifconfig enp0s31f6", "r");//运行脚本文件，获得返回值，装入buffer，方法1
    ...
    if(buffer[i+1]=='R'){
         printf("设备已经插入\n");
         //post_message();//设备启动，发送post给设备管理为服务
         break;
    }
    usleep(1000000);
    }
    motor_control();//开启电机控制
    return 0;
}